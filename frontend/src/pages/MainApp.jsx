...
export const MainApp=({user,setUser,goAuth,darkMode=true,setDarkMode})=>{
 ...
 return(
 <div style={{fontFamily:"'DM Sans',sans-serif",background:"#f7f7f8",color:"#111",minHeight:"100vh"}}><style>{`
 ...
 *{margin:0;padding:0;box-sizing:border-box;}
 ...
 {/* FIXED LEFT SIDEBAR */}
 <div className="fixed-left-sidebar" style={{position:"fixed",left:0,top:0,bottom:0,width:220,background:"#fff",borderRight:"1px solid rgba(0,0,0,0.08)",display:"flex",flexDirection:"column",padding:"20px 20px",zIndex:110,overflowY:"auto"}}><div onClick={()=>setPage("shop")} style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",flexShrink:0,marginBottom:26}}><span style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:19,letterSpacing:"-0.5px",color:"#111"}}>ShopTok</span></div><button onClick={()=>setPage("seller")} style={{display:"flex",alignItems:"center",gap:10,background:"none",border:"none",cursor:"pointer",color:"#111",fontSize:14,fontFamily:"inherit",padding:"9px 0",textAlign:"left"}}>Sell
 ...
 {!user&&(
 <button onClick={()=>goAuth(S.LOGIN)} style={{width:"100%",background:"#fe2c55",color:"#fff",border:"none",padding:"10px",borderRadius:100,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"Syne,sans-serif",marginTop:14}}>Log in</button>
 )}
 {user&&(
 ...
 <div key={l} style={{textAlign:"center",padding:"12px 8px",background:"rgba(0,0,0,0.03)",borderRadius:10,border:`1px solid ${c}20`}}><p style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:24,color:c,marginBottom:3}}>{v}</p><p style={{fontSize:11,color:"#555"}}>{l}</p></div>
 ))}
 ...
 <div key={l} style={{background:"#ffffff",border:"1px solid #1a1a1a",borderRadius:12,padding:16,textAlign:"center"}}><p style={{fontSize:11,color:"#555",marginBottom:6}}>{l}</p><p style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:16,color:c}}>Rs {v.toLocaleString()}</p></div>
 ))}
 </div>
 ...
 <div key={i} style={{background:"#ffffff",border:"1px solid #1a1a1a",borderRadius:12,padding:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><p style={{fontSize:13,fontWeight:600,marginBottom:2}}>{o.id}</p><p style={{fontSize:11,color:"#555"}}>{o.date}</p></div><div style={{textAlign:"right"}}><p style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:14,color:"#34d399"}}>+Rs {o.total.toLocaleString()}</p></div></div>
 ))}</div>}
 </div>}
 ...
 onMouseLeave={e=>e.currentTarget.style.background="transparent"}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:22}}>{o.items?o.items[0]?.emoji:o.product_emoji||""}</span><div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:13,color:"#fe2c55"}}>{o.id||o.order_number}</span><span style={{fontSize:11,color:"#555"}}>{o.date||new Date(o.created_at).toLocaleDateString("en-PK",{day:"2-digit",month:"short",year:"numeric"})}</span></div><p style={{fontSize:12,color:"rgba(0,0,0,0.5)",marginTop:2}}>{o.items?o.items[0]?.title:o.product_title}</p></div></div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:11,fontWeight:700,color:o.statusColor||"#fbbf24",background:(o.statusColor||"#fbbf24")+"18",padding:"4px 10px",borderRadius:100}}>● {o.status}</span><span style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:13,color:"#34d399"}}>Rs {Number(o.total||o.total_amount||0).toLocaleString()}</span><span style={{fontSize:11,color:"#555"}}>{expandedOrder===i?"▲":"▼"}</span></div></div>
 {expandedOrder===i&&(
 ...
 <div key={l} style={{background:"#ffffff",border:"1px solid #1a1a1a",borderRadius:14,padding:18}}><p style={{fontSize:11,color:"#555",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.05em"}}>{l}</p><p style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:22,color:c}}>{v}</p></div>
 ))}
 </div>
 ...
 style={{padding:"12px 8px",background:"rgba(52,211,153,0.08)",border:"1px solid rgba(52,211,153,0.25)",borderRadius:10,color:"#34d399",cursor:"pointer",fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:12,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}
 ...
 {[5,4,3,2,1].map(s=>(
 <div key={s} style={{background:"rgba(254,44,85,0.05)",border:"1px solid rgba(254,44,85,0.15)",borderRadius:10,padding:"12px 8px"}}><p style={{fontSize:18,marginBottom:4}}>{"⭐".repeat(s)}</p><p style={{fontFamily:"Syne,sans-serif",fontWeight:700,color:"#fe2c55"}}>0</p></div>
 ))}
 ...
 <div key={l} style={{background:"rgba(0,0,0,0.03)",border:"1px solid rgba(0,0,0,0.07)",borderRadius:12,padding:16,textAlign:"center"}}><p style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:24,color:c,marginBottom:4}}>{v}</p><p style={{fontSize:11,color:"#555"}}>{l}</p></div>
 ))}
 </div>
 {sellerProds.length===0
 ...
 {[
 [totalOrders,"Total Orders","#25f4ee"],
 [inProcess,"In Process","#fbbf24"],
 [completed,"Completed","#34d399"],
 [cancelled,"Cancelled","#fe2c55"],
 ].map(([val,label,color])=>(
 <div key={label} style={{textAlign:"center",padding:"14px 10px",background:"rgba(0,0,0,0.03)",borderRadius:12,border:`1px solid ${color}20`}}><p style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:28,color,marginBottom:4}}>{val}</p><p style={{fontSize:11,color:"#555"}}>{label}</p></div>
 ))}
 </div><div style={{padding:"16px 20px",borderTop:"1px solid rgba(0,0,0,0.06)",display:"flex",justifyContent:"center"}}><div style={{width:80,height:80,borderRadius:"50%",border:"2px solid #25f4ee",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:2}}><p style={{fontSize:10,color:"#25f4ee",fontWeight:700}}></p><p style={{fontSize:12,color:"#25f4ee",fontWeight:800}}>Verified</p><p style={{fontSize:10,color:"#25f4ee",fontWeight:700}}></p></div></div></div></div>
 );
 })()}
 ...
 {/* Header */}
 <div style={{display:"flex",alignItems:"center",gap:12,padding:"16px 0 20px"}}><button onClick={()=>setST("overview")} style={{background:"rgba(0,0,0,0.06)",border:"none",color:"#111",width:32,height:32,borderRadius:"50%",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>←</button><div><h2 style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:18}}>Store Setting</h2><p style={{fontSize:12,color:"rgba(0,0,0,0.4)"}}>Filters · Store Setting</p></div></div>

 {/* Store Information */}
 <div style={{background:"#ffffff",border:"1px solid #1a1a1a",borderRadius:14,padding:22,marginBottom:14}}><h3 style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:14,marginBottom:18,color:"#111"}}>Store Information</h3>
 ...
 style={{background:"linear-gradient(135deg,#fe2c55,#ff6b35)",border:"none",color:"#fff",padding:"10px 28px",borderRadius:8,fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:13,cursor:"pointer"}}>
 {storeSaving?"Saving...":"Save"}
 </button></div></div>

 {/* Banner Settings */}
 <div style={{background:"#ffffff",border:"1px solid #1a1a1a",borderRadius:14,padding:22,marginBottom:14}}><h3 style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:14,marginBottom:18,color:"#111"}}>Banner Settings</h3>
 ...
 <div style={{background:"#ffffff",border:"1px solid #1a1a1a",borderRadius:14,padding:22,marginBottom:14}}><h3 style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:14,marginBottom:18,color:"#111"}}>Personal Information</h3>
 ...
 {/* VENTURE ALLIANCE */}
 {sellerTab==="venture"&&<div><div style={{background:"linear-gradient(135deg,#0a0a1a,#1a0a2e)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:20,padding:28,marginBottom:16,textAlign:"center"}}><p style={{fontSize:40,marginBottom:12}}></p><h2 style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:22,marginBottom:8,color:"#a78bfa"}}>Venture Alliance Program</h2><p style={{fontSize:13,color:"rgba(255,255,255,0.65)",lineHeight:1.7,marginBottom:20}}>Partner with ShopTok and grow your business. Earn commissions, get priority support, and access exclusive seller benefits.</p><button style={{padding:"12px 28px",background:"linear-gradient(135deg,#a78bfa,#6d28d9)",border:"none",borderRadius:12,color:"#fff",fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:14,cursor:"pointer"}}>Join Alliance →</button></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
 ...
 {/* BUYER PROFILE */}
 {page==="profile"&&(
 <div style={{display:"flex",width:"100%",minHeight:"calc(100vh - 60px)"}}><Sidebar user={user} profileImg={profileImg} tab={profileTab} setTab={setPT} onAddProduct={()=>{}} onLogout={logout} onEditProfile={()=>setPE(true)} tabs={BUYER_TABS} showAdd={false}/><div style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"26px 28px 60px",minWidth:0}}><div style={{marginBottom:18}}><h1 style={{fontFamily:"Syne,sans-serif",fontWeight:800,fontSize:22,marginBottom:3}}>
